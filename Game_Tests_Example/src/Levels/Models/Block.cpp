...
VertArray* Block::vao = (VertArray*)NULL;

Block::Block(unsigned short ID)
{
	this->ID = ID;
	if (vao == (VertArray*)NULL) 
	{
		CORE_INFO("Initializeing game...");
		init();
		CORE_INFO("Game initialized!");
	}
	srand((unsigned int) time(NULL));
	color = Maths::vec3(0.3, 0.6, 0.2);
	shader = Shaders::BLOCK;
	CORE_INFO("Creating blocks...");
}
...
void Block::render(const Maths::vec3& position)
{
	shader->enable();
	texture->enable();
	Maths::mat4 pos = Maths::mat4::Translate(position);
...
void Block::init()
{
	...
	vao = new VertArray(vertices, indices, normals, tcs, 6 * 4 * 3, 6 * 6);
}
