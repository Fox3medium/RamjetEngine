...
Level::Level()
{
	add(new Player(vec3(-45.0f, 48.0f, -35.0f)));
...
void Level::select_block(const Maths::vec3& block)
{
	this->selected_block = block;
}

unsigned short& Level::get_intersection_block(Entity* entity)
{
...
unsigned short& Level::get_block(const Maths::vec3& position)
{
...
unsigned short& Level::pick_block(const Maths::vec3& position, const Maths::vec3& rotation)
{
	const float MAX_DISTANCE = 32.0f;
	vec3 rot = vec3(Maths::cos(toRadians(rotation.y - 90.0f)), -Maths::tan(toRadians(rotation.x)), Maths::sin(toRadians(rotation.y - 90.0f)));
	return raycast_block_id(position, rot, MAX_DISTANCE);
}

unsigned short& Level::raycast_collision(const Maths::vec3& position, const Maths::vec3& rotation)
{
...
unsigned short& Level::raycast_block_id(const Maths::vec3& position, const Maths::vec3& rotation)
{
	const float MAX_DISTANCE = 32.0f;
	return raycast_block_id(position, rotation, MAX_DISTANCE);
}

unsigned short& Level::raycast_block_id(const Maths::vec3& position, const Maths::vec3& rotation, float distance)
{
	vec3 pos = vec3(-position.x, -position.y, -position.z);
	const float ITERATION = 0.5f;
...
unsigned short& Level::raycast_pre_block_id(const Maths::vec3& position, const Maths::vec3& rotation)
{
	vec3 pos = vec3(-position.x, -position.y, -position.z);
	vec3 ray_vector = vec3(Maths::cos(toRadians(rotation.y - 90.0f)), -Maths::tan(toRadians(rotation.x)), Maths::sin(toRadians(rotation.y - 90.0f)));
	const float MAX_DISTANCE = 32.0f;
	const float ITERATION = 0.5f;
	float distance = 0.0f;
	while (distance < MAX_DISTANCE)
...
Maths::vec3 Level::raycast_block(const Maths::vec3& position, const Maths::vec3& rotation)
{
	vec3 pos = vec3(-position.x, -position.y, -position.z);
	vec3 ray_vector = vec3(Maths::cos(toRadians(rotation.y - 90.0f)), -Maths::tan(toRadians(rotation.x)), Maths::sin(toRadians(rotation.y - 90.0f)));
	const float MAX_DISTANCE = 32.0f;
